...
2. Para controlar o acesso de usuário a determinadas funções ou propriedades, chame [Export-ModuleMember](/powershell/module/Microsoft.PowerShell.Core/Export-ModuleMember) no final do seu script.
...
4. Para descrever seu módulo para o sistema de Ajuda do PowerShell, você pode usar comentários ajuda padrão dentro do arquivo ou criar um arquivo de ajuda adicional.

   O exemplo de código na parte inferior deste tópico inclui as informações da Ajuda nos comentários. Você também pode escrever arquivos expandidos de XML que contêm o conteúdo de ajuda adicional. Para obter mais informações, consulte [Ajuda do escrita para o Windows PowerShell módulos](./writing-help-for-windows-powershell-modules.md).
...
O exemplo a seguir é um módulo de script simples que contém uma única chamada de função `Show-Calendar`.
Esta função exibe uma representação visual de um calendário. Além disso, o exemplo contém as cadeias de caracteres de Ajuda do PowerShell para a Sinopse, descrição, valores de parâmetro e exemplo. Observe que a última linha do código garante que o `Show-Calendar` função é exportada como um membro de módulo quando o módulo é importado.
