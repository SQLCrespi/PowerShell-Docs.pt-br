...
## <a name="why-do-dependency-conflicts-occur"></a><span data-ttu-id="de33c-112">Por que conflitos de dependência ocorrem?</span><span class="sxs-lookup"><span data-stu-id="de33c-112">Why do dependency conflicts occur?</span></span>
...
<span data-ttu-id="de33c-155">Neste artigo, vamos nos referir aos seguintes mecanismos:</span><span class="sxs-lookup"><span data-stu-id="de33c-155">In this article we'll refer to the following mechanisms:</span></span>
...
<span data-ttu-id="de33c-187">Por exemplo, a seguir estão duas versões do mesmo código que carregam uma dependência em momentos diferentes.</span><span class="sxs-lookup"><span data-stu-id="de33c-187">For example, here are two versions of the same code that load a dependency at different times.</span></span>
...
<span data-ttu-id="de33c-219">As circunstâncias em que anexar uma versão de dependência comum não funciona incluem:</span><span class="sxs-lookup"><span data-stu-id="de33c-219">Circumstances where pinning to a common dependency version won't work include:</span></span>
...
- <span data-ttu-id="de33c-253">Comunicação remota implícita com o Windows PowerShell</span><span class="sxs-lookup"><span data-stu-id="de33c-253">Implicit remoting to Windows PowerShell</span></span>
...
## <a name="more-robust-solutions"></a><span data-ttu-id="de33c-266">Soluções mais robustas</span><span class="sxs-lookup"><span data-stu-id="de33c-266">More robust solutions</span></span>
...
<span data-ttu-id="de33c-306">Para ver como a implementação muda, começaremos com a implementação do `AlcModule.Engine.dll`:</span><span class="sxs-lookup"><span data-stu-id="de33c-306">To see how the implementation changes, we'll start with the implementation of `AlcModule.Engine.dll`:</span></span>
...
<span data-ttu-id="de33c-311">Agora, precisamos adicionar o código ao `AlcModule.Cmdlets.dll` para que ele saiba como resolver o `AlcModule.Engine.dll`.</span><span class="sxs-lookup"><span data-stu-id="de33c-311">Now we need to add code to `AlcModule.Cmdlets.dll` so that it knows how to resolve `AlcModule.Engine.dll`.</span></span> <span data-ttu-id="de33c-312">Primeiro, precisamos definir nosso ALC personalizado que resolverá assemblies do diretório `Dependencies` do módulo:</span><span class="sxs-lookup"><span data-stu-id="de33c-312">First we must define our custom ALC that will resolve assemblies from our module's `Dependencies` directory:</span></span>
...
<span data-ttu-id="de33c-327">Então, quando criamos o módulo, nossa estratégia é:</span><span class="sxs-lookup"><span data-stu-id="de33c-327">So, when we build the module, our strategy is:</span></span>
...
<span data-ttu-id="de33c-343">Assim como no módulo do ALC, precisamos primeiro dividir o módulo em duas partes.</span><span class="sxs-lookup"><span data-stu-id="de33c-343">As with the ALC module, we must first split up the module into two pieces.</span></span>
...
<span data-ttu-id="de33c-348">Por fim, o layout do módulo também é semelhante ao módulo do ALC:</span><span class="sxs-lookup"><span data-stu-id="de33c-348">Finally, the layout of the module is also similar to the ALC module:</span></span>
...
<span data-ttu-id="de33c-375">Se você está interessado em tentar usar um Domínio do Aplicativo personalizado, os seguintes links podem ser úteis:</span><span class="sxs-lookup"><span data-stu-id="de33c-375">If you're interested in trying to use a custom Application Domain, the following links might help:</span></span>
...
<span data-ttu-id="de33c-383">Se você sabe que o seu módulo só é usado em ambientes do Windows PowerShell 5.1 dos quais você tem controle total, algumas dessas opções podem ser usadas.</span><span class="sxs-lookup"><span data-stu-id="de33c-383">If you know that your module is only used in Windows PowerShell 5.1 environments that you have total control over, some of these may be options.</span></span> <span data-ttu-id="de33c-384">No entanto, em geral, **os módulos não devem modificar o estado do computador global dessa forma**.</span><span class="sxs-lookup"><span data-stu-id="de33c-384">In general however, **modules should not modify global machine state like this**.</span></span> <span data-ttu-id="de33c-385">Isso pode interromper as configurações que causam problemas em `powershell.exe`, em outros módulos ou em outros aplicativos dependentes o que faz com que o módulo falhe de maneiras inesperadas.</span><span class="sxs-lookup"><span data-stu-id="de33c-385">It can break configurations that cause problems in `powershell.exe`, other modules, or other dependent applications that cause your module to fail in unexpected ways.</span></span>

### <a name="static-binding-redirect-with-appconfig-to-force-using-the-same-dependency-version"></a><span data-ttu-id="de33c-386">Redirecionamento de associação estática com app.config para forçar o uso da mesma versão de dependência</span><span class="sxs-lookup"><span data-stu-id="de33c-386">Static binding redirect with app.config to force using the same dependency version</span></span>

<span data-ttu-id="de33c-387">Os aplicativos .NET Framework podem aproveitar um arquivo `app.config` para configurar alguns dos comportamentos do aplicativo de modo declarativo.</span><span class="sxs-lookup"><span data-stu-id="de33c-387">.NET Framework applications can take advantage of an `app.config` file to configure some of the application's behaviors declaratively.</span></span> <span data-ttu-id="de33c-388">É possível gravar uma entrada de `app.config` que configura a associação de assembly para redirecionar o carregamento de assembly para uma versão específica.</span><span class="sxs-lookup"><span data-stu-id="de33c-388">It's possible to write an `app.config` entry that configures assembly binding to redirect assembly loading to a particular version.</span></span>

<span data-ttu-id="de33c-389">Essa solução no PowerShell tem dois problemas:</span><span class="sxs-lookup"><span data-stu-id="de33c-389">Two issues with this for PowerShell are:</span></span>

- <span data-ttu-id="de33c-390">O .NET Core não dá suporte a `app.config`, portanto, essa solução só se aplica a `powershell.exe`.</span><span class="sxs-lookup"><span data-stu-id="de33c-390">.NET Core does not support `app.config`, so this solution only applies to `powershell.exe`.</span></span>
- <span data-ttu-id="de33c-391">`powershell.exe` é um aplicativo compartilhado que reside no diretório `System32`.</span><span class="sxs-lookup"><span data-stu-id="de33c-391">`powershell.exe` is a shared application that lives in the `System32` directory.</span></span> <span data-ttu-id="de33c-392">É provável que o seu módulo não seja capaz de modificar o conteúdo dele em muitos sistemas.</span><span class="sxs-lookup"><span data-stu-id="de33c-392">It's likely that your module won't be able to modify its contents on many systems.</span></span> <span data-ttu-id="de33c-393">Mesmo que ele seja, modificar o `app.config` pode interromper uma configuração existente ou afetar o carregamento de outros módulos.</span><span class="sxs-lookup"><span data-stu-id="de33c-393">Even if it can, modifying the `app.config` could break an existing configuration or affect the loading of other modules.</span></span>

### <a name="setting-codebase-with-appconfig"></a><span data-ttu-id="de33c-394">Configurar o `codebase` com app.config</span><span class="sxs-lookup"><span data-stu-id="de33c-394">Setting `codebase` with app.config</span></span>

<span data-ttu-id="de33c-395">Pelos mesmos motivos, tentar definir a configuração de `codebase` em `app.config` não funcionará nos módulos do PowerShell.</span><span class="sxs-lookup"><span data-stu-id="de33c-395">For the same reasons, trying to configure the `codebase` setting in `app.config` is not going to work in PowerShell modules.</span></span>

### <a name="installing-dependencies-to-the-global-assembly-cache-gac"></a><span data-ttu-id="de33c-396">Instalar dependências no GAC (Cache de Assembly Global)</span><span class="sxs-lookup"><span data-stu-id="de33c-396">Installing dependencies to the Global Assembly Cache (GAC)</span></span>

<span data-ttu-id="de33c-397">Outra maneira de resolver conflitos de versão de dependência no .NET Framework é instalar dependências no GAC, de modo que versões diferentes possam ser carregadas lado a lado no GAC.</span><span class="sxs-lookup"><span data-stu-id="de33c-397">Another way to resolve dependency version conflicts in .NET Framework is to install dependencies to the GAC, so that different versions can be loaded side-by-side from the GAC.</span></span>

<span data-ttu-id="de33c-398">Novamente, nos módulos do PowerShell os principais problemas são:</span><span class="sxs-lookup"><span data-stu-id="de33c-398">Again, for PowerShell modules, the chief issues here are:</span></span>

- <span data-ttu-id="de33c-399">O GAC aplica-se somente ao .NET Framework, portanto, isso não funciona no PowerShell 6 e versões posteriores.</span><span class="sxs-lookup"><span data-stu-id="de33c-399">The GAC only applies to .NET Framework, so this does not help in PowerShell 6 and above.</span></span>
- <span data-ttu-id="de33c-400">A instalação de assemblies no GAC é uma modificação do estado do computador global e pode causar efeitos colaterais em outros aplicativos ou em outros módulos.</span><span class="sxs-lookup"><span data-stu-id="de33c-400">Installing assemblies to the GAC is a modification of global machine state and may cause side-effects in other applications or to other modules.</span></span> <span data-ttu-id="de33c-401">Ela também pode ser difícil realizar corretamente, mesmo quando o módulo tem os privilégios de acesso necessários.</span><span class="sxs-lookup"><span data-stu-id="de33c-401">It may also be difficult to do correctly, even when your module has the required access privileges.</span></span> <span data-ttu-id="de33c-402">Fazer algo errado pode causar problemas sérios e em todo o computador em outros aplicativos .NET.</span><span class="sxs-lookup"><span data-stu-id="de33c-402">Getting it wrong could cause serious, machine-wide issues in other .NET applications.</span></span>

## <a name="further-reading"></a><span data-ttu-id="de33c-403">Leitura adicional</span><span class="sxs-lookup"><span data-stu-id="de33c-403">Further reading</span></span>

<span data-ttu-id="de33c-404">Há muitos outros artigos sobre conflitos de dependência de versão do assembly do .NET que você pode ler.</span><span class="sxs-lookup"><span data-stu-id="de33c-404">There's plenty more to read on .NET assembly version dependency conflicts.</span></span> <span data-ttu-id="de33c-405">Veja alguns pontos de partida interessantes:</span><span class="sxs-lookup"><span data-stu-id="de33c-405">Here are some nice jumping off points:</span></span>

- [<span data-ttu-id="de33c-406">.NET: Assemblies no .NET</span><span class="sxs-lookup"><span data-stu-id="de33c-406">.NET: Assemblies in .NET</span></span>](/dotnet/standard/assembly/)
- [<span data-ttu-id="de33c-407">.NET Core: O algoritmo de carregamento de assembly gerenciado</span><span class="sxs-lookup"><span data-stu-id="de33c-407">.NET Core: The managed assembly loading algorithm</span></span>](/dotnet/core/dependency-loading/loading-managed)
- [<span data-ttu-id="de33c-408">.NET Core: Compreendendo o System.Runtime.Loader.AssemblyLoadContext</span><span class="sxs-lookup"><span data-stu-id="de33c-408">.NET Core: Understanding System.Runtime.Loader.AssemblyLoadContext</span></span>](/dotnet/core/dependency-loading/understanding-assemblyloadcontext)
- [<span data-ttu-id="de33c-409">.NET Core: Discussão sobre soluções de carregamento lado a lado do assembly</span><span class="sxs-lookup"><span data-stu-id="de33c-409">.NET Core: Discussion about side-by-side assembly loading solutions</span></span>](https://github.com/dotnet/runtime/issues/13471)
- [<span data-ttu-id="de33c-410">.NET Framework: Como redirecionar versões de assembly</span><span class="sxs-lookup"><span data-stu-id="de33c-410">.NET Framework: Redirecting assembly versions</span></span>](/dotnet/framework/configure-apps/redirect-assembly-versions)
- [<span data-ttu-id="de33c-411">.NET Framework: Práticas recomendadas para carregamento de assembly</span><span class="sxs-lookup"><span data-stu-id="de33c-411">.NET Framework: Best practices for assembly loading</span></span>](/dotnet/framework/deployment/best-practices-for-assembly-loading)
- [<span data-ttu-id="de33c-412">.NET Framework: Como o runtime localiza os assemblies</span><span class="sxs-lookup"><span data-stu-id="de33c-412">.NET Framework: How the runtime locates assemblies</span></span>](/dotnet/framework/deployment/how-the-runtime-locates-assemblies)
- [<span data-ttu-id="de33c-413">.NET Framework: Resolver carregamentos de assembly</span><span class="sxs-lookup"><span data-stu-id="de33c-413">.NET Framework: Resolve assembly loads</span></span>](/dotnet/standard/assembly/resolve-loads)
- [<span data-ttu-id="de33c-414">Stack Overflow: Como e por que realizar o redirecionamento da associação de assembly?</span><span class="sxs-lookup"><span data-stu-id="de33c-414">StackOverflow: Assembly binding redirect, how and why?</span></span>](https://stackoverflow.com/questions/43365736/assembly-binding-redirect-how-and-why)
- [<span data-ttu-id="de33c-415">PowerShell: Discussão sobre a implementação do AssemblyLoadContexts</span><span class="sxs-lookup"><span data-stu-id="de33c-415">PowerShell: Discussion about implementing AssemblyLoadContexts</span></span>](https://github.com/PowerShell/PowerShell/issues/11571)
- [<span data-ttu-id="de33c-416">PowerShell: `Assembly.LoadFile()` não carrega no AssemblyLoadContext padrão</span><span class="sxs-lookup"><span data-stu-id="de33c-416">PowerShell: `Assembly.LoadFile()` doesn't load into default AssemblyLoadContext</span></span>](https://github.com/PowerShell/PowerShell/issues/12052)
- [<span data-ttu-id="de33c-417">Rick Strahl: Quando uma dependência de assembly do .NET é carregada?</span><span class="sxs-lookup"><span data-stu-id="de33c-417">Rick Strahl: When does a .NET assembly dependency get loaded?</span></span>](https://weblog.west-wind.com/posts/2012/Nov/03/Back-to-Basics-When-does-a-NET-Assembly-Dependency-get-loaded)
- [<span data-ttu-id="de33c-418">Jon Skeet: Resumo do controle de versão no .NET</span><span class="sxs-lookup"><span data-stu-id="de33c-418">Jon Skeet: Summary of versioning in .NET</span></span>](https://codeblog.jonskeet.uk/2019/06/30/versioning-limitations-in-net/)
- [<span data-ttu-id="de33c-419">Nate McMaster: Saiba mais sobre os primitivos do .NET Core</span><span class="sxs-lookup"><span data-stu-id="de33c-419">Nate McMaster: Deep dive into .NET Core primitives</span></span>](https://natemcmaster.com/blog/2017/12/21/netcore-primitives/)
