...
|Parâmetro|Descrição|
|---------------|-----------------|
|-out n. exe|Necessário. O nome do Shell a ser produzido. O caminho é especificado como parte desse parâmetro.<br /><br /> Make-Shell acrescentará ". exe" a esse valor se não for especificado. **Cuidado:**  Não crie um arquivo de saída com o mesmo nome que o arquivo. dll referenciado. Se você tentar isso, a ferramenta Make-shell criará um arquivo. cs com o mesmo nome, que substituirá o arquivo. cs que tem o código-fonte do cmdlet.|
|-namespace NS|Necessário. O namespace a ser usado para a classe derivada de [System. Management. Automation. Runspaces. Runspaceconfiguration](/dotnet/api/System.Management.Automation.Runspaces.RunspaceConfiguration) que o make-kit gera e compila.|
|-lib libdirectory1 [, libdirectory2,..]|Os diretórios que são pesquisados em assemblies do .NET, incluindo os assemblies do Windows PowerShell, os assemblies especificados pelo parâmetro `reference`, os assemblies referenciados indiretamente por outro assembly e os assemblies do sistema .NET.|
...
|-AuthorizationManager authorizationManagerType|O tipo que contém a implementação do Gerenciador de autorização. Isso pode ser definido no código-fonte ou compilado em um assembly (especificado pelo parâmetro `reference`). Se esse parâmetro não for especificado, o Gerenciador de segurança padrão será usado. O valor deve ser o nome completo do tipo, incluindo namespaces.|
