...
> [!NOTE]
> Os exemplos nas seções a seguir são do arquivo de tipos `Types.ps1xml` no diretório de instalação do Windows PowerShell (`$PSHOME`). Para obter mais informações, consulte [sobre tipos. ps1xml](/powershell/module/microsoft.powershell.core/about/about_types.ps1xml).
