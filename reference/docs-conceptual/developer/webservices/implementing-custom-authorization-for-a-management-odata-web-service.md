...
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="24d29-102">Implementar autorização personalizada para um serviço Web OData de gerenciamento</span><span class="sxs-lookup"><span data-stu-id="24d29-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="24d29-103">Usar o serviço Web do Windows PowerShell requer um terceiro para implementar a interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) para expor os cmdlets do Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="24d29-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="24d29-104">Essa interface executa a autorização do usuário para o serviço Web.</span><span class="sxs-lookup"><span data-stu-id="24d29-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="24d29-105">Depois de escrever o código para implementar a interface, você deve compilá-lo em uma DLL a ser usada no aplicativo Web.</span><span class="sxs-lookup"><span data-stu-id="24d29-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="24d29-106">Autorização de passagem</span><span class="sxs-lookup"><span data-stu-id="24d29-106">Pass-through authorization</span></span>

<span data-ttu-id="24d29-107">A maneira mais simples de implementar a interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) é uma implementação de passagem que autoriza todos os usuários.</span><span class="sxs-lookup"><span data-stu-id="24d29-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="24d29-108">Este exemplo não fornece segurança, e os s são fornecidos apenas como uma ilustração de como implementar a interface.</span><span class="sxs-lookup"><span data-stu-id="24d29-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="24d29-109">Uma implementação da interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) deve substituir dois métodos: [Microsoft. Management. OData. CustomAuthorization. AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) e [Microsoft. Management. OData. CustomAuthorization.](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId)getassociaid.</span><span class="sxs-lookup"><span data-stu-id="24d29-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="24d29-110">Neste exemplo, o [Microsoft. Management. OData. CustomAuthorization. AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) sempre retorna o objeto **System. Security. principal. WindowsIdentity** associado ao usuário atual.</span><span class="sxs-lookup"><span data-stu-id="24d29-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="24d29-111">Autorização baseada em função</span><span class="sxs-lookup"><span data-stu-id="24d29-111">Role-based authorization</span></span>

<span data-ttu-id="24d29-112">O exemplo a seguir implementa uma política de autorização baseada em função.</span><span class="sxs-lookup"><span data-stu-id="24d29-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="24d29-113">A política é definida em um arquivo XML que reside no diretório principal do aplicativo com os arquivos de esquema Web. config e MOF e de mapeamento XML.</span><span class="sxs-lookup"><span data-stu-id="24d29-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="24d29-114">Para obter informações sobre como configurar o arquivo de esquema de autorização, consulte [Configurando a autorização baseada em função](./configuring-role-based-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="24d29-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="24d29-115">A primeira parte do exemplo implementa os métodos [Microsoft. Management. OData. CustomAuthorization. AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) e [Microsoft. Management. OData. CustomAuthorization. getafiliaid](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) .</span><span class="sxs-lookup"><span data-stu-id="24d29-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="24d29-116">Nesse caso, os métodos de interface chamam métodos na `RbacSystem` classe (definida abaixo) que fazem o trabalho real de verificar as permissões para o usuário.</span><span class="sxs-lookup"><span data-stu-id="24d29-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="24d29-117">O exemplo a seguir cria uma classe que analisa a política de autorização no arquivo XML.</span><span class="sxs-lookup"><span data-stu-id="24d29-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="24d29-118">As classes a seguir representam grupos e usuários, respectivamente.</span><span class="sxs-lookup"><span data-stu-id="24d29-118">The following classes represent groups and users, respectively.</span></span>
...
<span data-ttu-id="24d29-119">Por fim, a classe RbacSystem implementa métodos que realizam o trabalho de verificar as permissões para o usuário e retornam o status de autorização para os métodos definidos na implementação da interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) .</span><span class="sxs-lookup"><span data-stu-id="24d29-119">Finally, the RbacSystem class implements methods that do the work of checking the permissions for the user and return the authorization status to the methods defined in the implementation of the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface.</span></span>
