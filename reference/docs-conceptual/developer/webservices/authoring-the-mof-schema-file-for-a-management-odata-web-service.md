...
Você define os recursos que o serviço Web do Management OData expõe criando um arquivo MOF que usou o esquema de recursos públicos. Cada recurso é definido como uma classe no arquivo e as propriedades são definidas como membros da classe. Para obter mais informações sobre o esquema usado no arquivo MOF, consulte [esquema de recurso público](./public-resource-schema.md).
...
Cada um dos dois recursos contém propriedades que são de tipo complexo. Os tipos complexos são definidos como classes modificadas com o `ComplexType` qualificador.
