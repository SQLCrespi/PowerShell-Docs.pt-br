...
3. Em cada elemento `DynamicParameter`, adicione um elemento `Name` e `CmdletSupported`.
   ...
   Por exemplo, o XML a seguir documenta o parâmetro dinâmico `Encoding` que o provedor do sistema de arquivos do Windows PowerShell adiciona aos cmdlets `Add-Content`, `Get-Content` `Set-Content`.
...
6. Adicione o elemento `PossibleValues` e seus elementos filho. Juntos, esses elementos descrevem os valores do parâmetro dinâmico. Esse elemento é projetado para valores enumerados. Se o parâmetro dinâmico não receber um valor, como é o caso com um parâmetro de opção, ou os valores não podem ser enumerados, adicione um elemento `PossibleValues` vazio.
   ...
   |Descrição|Este elemento contém um elemento `Para`. O texto no elemento `Para` descreve o valor nomeado no elemento `Value`.|

   Por exemplo, o XML a seguir mostra um elemento `PossibleValue` do parâmetro dinâmico `Encoding`.
...
O exemplo a seguir mostra o elemento `DynamicParameters` do parâmetro dinâmico `Encoding`.
