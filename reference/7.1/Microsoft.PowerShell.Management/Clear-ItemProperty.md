...
## SINOPSE
Limpa o valor de uma propriedade, mas não exclui a propriedade.
...
O `Clear-ItemProperty` cmdlet limpa o valor de uma propriedade, mas não exclui a propriedade.
Você pode usar este cmdlet para excluir os dados de um valor do registro.

## EXEMPLOS

### Exemplo 1: limpar o valor da chave do registro

Esse comando limpa os dados no valor do registro "Options" na subchave "MyApp" de `HKEY_LOCAL_MACHINE\Software\MyCompany` .
...
Indica que esse cmdlet exclui Propriedades de itens que não podem ser acessados pelo usuário de outra forma. A implementação varia de provedor para provedor.
...
Especifica o nome da propriedade a ser apagada, como o nome de um valor do registro.
...
Especifica o caminho para as propriedades sendo apagadas.
Caracteres curinga são permitidos.
...
Quando você usa o parâmetro **PassThru** , o `Clear-ItemProperty` gera um objeto **PSCustomObject** que representa a propriedade item limpa. Caso contrário, este cmdlet não gera nenhuma saída.

## OBSERVAÇÕES

- Você pode usar `Clear-ItemProperty` para excluir os dados em valores de registro sem excluir o valor.
  Se o tipo de dados do valor for binários ou DWORD, limpar os dados definirá o valor como zero.
  Caso contrário, o valor ficará vazio.
- O `Clear-ItemProperty` cmdlet é projetado para trabalhar com os dados expostos por qualquer provedor. Para listar os provedores disponíveis em sua sessão, digite `Get-PSProvider` . Para obter mais informações, consulte [about_Providers](../Microsoft.PowerShell.Core/About/about_Providers.md).
