...
<span data-ttu-id="53fa8-112">A tabela a seguir lista as variáveis de preferência e seus valores padrão.</span><span class="sxs-lookup"><span data-stu-id="53fa8-112">The following table lists the preference variables and their default values.</span></span>
...
<span data-ttu-id="53fa8-176">A estimativa do risco é um atributo do cmdlet ou da função conhecida como seu **ConfirmImpact** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-176">The estimate of the risk is an attribute of the cmdlet or function known as its **ConfirmImpact** .</span></span> <span data-ttu-id="53fa8-177">Os usuários não podem alterá-la.</span><span class="sxs-lookup"><span data-stu-id="53fa8-177">Users can't change it.</span></span>
...
<span data-ttu-id="53fa8-196">Os valores válidos são os seguintes:</span><span class="sxs-lookup"><span data-stu-id="53fa8-196">The valid values are as follows:</span></span>
...
<span data-ttu-id="53fa8-218">Este exemplo usa o parâmetro **debug** com um valor de `$false` para suprimir a mensagem para um único comando.</span><span class="sxs-lookup"><span data-stu-id="53fa8-218">This example uses the **Debug** parameter with a value of `$false` to suppress the message for a single command.</span></span> <span data-ttu-id="53fa8-219">A mensagem de depuração não é exibida e o processamento não é interrompido.</span><span class="sxs-lookup"><span data-stu-id="53fa8-219">The debug message isn't displayed and processing isn't stopped.</span></span>
...
<span data-ttu-id="53fa8-228">Os valores válidos são os seguintes:</span><span class="sxs-lookup"><span data-stu-id="53fa8-228">The valid values are as follows:</span></span>
...
#### <a name="examples"></a><span data-ttu-id="53fa8-246">Exemplos</span><span class="sxs-lookup"><span data-stu-id="53fa8-246">Examples</span></span>
...
<span data-ttu-id="53fa8-260">Os valores válidos são os seguintes:</span><span class="sxs-lookup"><span data-stu-id="53fa8-260">The valid values are as follows:</span></span>
...
<span data-ttu-id="53fa8-275">Este exemplo mostra como um erro aparece quando o valor de `$ErrorView` é o padrão, **ConciseView** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-275">This example shows how an error appears when the value of `$ErrorView` is the default, **ConciseView** .</span></span> <span data-ttu-id="53fa8-276">`Get-ChildItem` é usado para localizar um diretório não existente.</span><span class="sxs-lookup"><span data-stu-id="53fa8-276">`Get-ChildItem` is used to find a non-existent directory.</span></span>
...
<span data-ttu-id="53fa8-321">As variáveis de preferência de \*\*evento log \*\*\* são as seguintes:</span><span class="sxs-lookup"><span data-stu-id="53fa8-321">The **Log\*Event** preference variables are as follows:</span></span>
...
- <span data-ttu-id="53fa8-386">**SilentlyContinue** : executa o comando, mas não exibe a barra de progresso.</span><span class="sxs-lookup"><span data-stu-id="53fa8-386">**SilentlyContinue** : Executes the command, but doesn't display the progress bar.</span></span>
...
<span data-ttu-id="53fa8-400">Os valores válidos são:</span><span class="sxs-lookup"><span data-stu-id="53fa8-400">Valid values are:</span></span>
...
<span data-ttu-id="53fa8-425">O valor padrão `http://schemas.microsoft.com/PowerShell/microsoft.PowerShell` indica a configuração de sessão do **Microsoft. PowerShell** no computador remoto.</span><span class="sxs-lookup"><span data-stu-id="53fa8-425">The default value `http://schemas.microsoft.com/PowerShell/microsoft.PowerShell` indicates the **Microsoft.PowerShell** session configuration on the remote computer.</span></span>
...
<span data-ttu-id="53fa8-428">Você pode alterar o valor dessa variável a qualquer momento.</span><span class="sxs-lookup"><span data-stu-id="53fa8-428">You can change the value of this variable at any time.</span></span> <span data-ttu-id="53fa8-429">Quando você fizer isso, lembre-se de que a configuração de sessão selecionada deve existir no computador remoto.</span><span class="sxs-lookup"><span data-stu-id="53fa8-429">When you do, remember that the session configuration that you select must exist on the remote computer.</span></span> <span data-ttu-id="53fa8-430">Caso contrário, o comando para criar uma sessão que usa a configuração de sessão falhará.</span><span class="sxs-lookup"><span data-stu-id="53fa8-430">If it doesn't, the command to create a session that uses the session configuration fails.</span></span>

<span data-ttu-id="53fa8-431">Essa variável de preferência não determina quais configurações de sessão local são usadas quando usuários remotos criam uma sessão que se conecta a este computador.</span><span class="sxs-lookup"><span data-stu-id="53fa8-431">This preference variable doesn't determine which local session configurations are used when remote users create a session that connects to this computer.</span></span>
<span data-ttu-id="53fa8-432">No entanto, você pode usar as permissões para as configurações de sessão local para determinar quais usuários podem usá-las.</span><span class="sxs-lookup"><span data-stu-id="53fa8-432">However, you can use the permissions for the local session configurations to determine which users may use them.</span></span>

### <a name="pssessionoption"></a><span data-ttu-id="53fa8-433">\$PSSessionOption</span><span class="sxs-lookup"><span data-stu-id="53fa8-433">\$PSSessionOption</span></span>

<span data-ttu-id="53fa8-434">Estabelece os valores padrão para opções de usuário avançadas em uma sessão remota.</span><span class="sxs-lookup"><span data-stu-id="53fa8-434">Establishes the default values for advanced user options in a remote session.</span></span>
<span data-ttu-id="53fa8-435">Essas preferências de opção substituem os valores padrão do sistema para as opções de sessão.</span><span class="sxs-lookup"><span data-stu-id="53fa8-435">These option preferences override the system default values for session options.</span></span>

<span data-ttu-id="53fa8-436">A `$PSSessionOption` variável contém um objeto **PSSessionOption** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-436">The `$PSSessionOption` variable contains a **PSSessionOption** object.</span></span> <span data-ttu-id="53fa8-437">Para obter mais informações, consulte [System. Management. Automation. Remoting. PSSessionOption](/dotnet/api/system.management.automation.remoting.pssessionoption).</span><span class="sxs-lookup"><span data-stu-id="53fa8-437">For more information, see [System.Management.Automation.Remoting.PSSessionOption](/dotnet/api/system.management.automation.remoting.pssessionoption).</span></span>
<span data-ttu-id="53fa8-438">Cada propriedade do objeto representa uma opção de sessão.</span><span class="sxs-lookup"><span data-stu-id="53fa8-438">Each property of the object represents a session option.</span></span> <span data-ttu-id="53fa8-439">Por exemplo, a propriedade **NoCompression** desativa a compactação de dados durante a sessão.</span><span class="sxs-lookup"><span data-stu-id="53fa8-439">For example, the **NoCompression** property turns of data compression during the session.</span></span>

<span data-ttu-id="53fa8-440">Por padrão, a `$PSSessionOption` variável contém um objeto **PSSessionOption** com os valores padrão para todas as opções, como mostrado abaixo.</span><span class="sxs-lookup"><span data-stu-id="53fa8-440">By default, the `$PSSessionOption` variable contains a **PSSessionOption** object with the default values for all options, as shown below.</span></span>
...
<span data-ttu-id="53fa8-441">Para obter descrições dessas opções e mais informações, consulte [New-PSSessionOption](xref:Microsoft.PowerShell.Core.New-PSSessionOption).</span><span class="sxs-lookup"><span data-stu-id="53fa8-441">For descriptions of these options and more information, see [New-PSSessionOption](xref:Microsoft.PowerShell.Core.New-PSSessionOption).</span></span> <span data-ttu-id="53fa8-442">Para obter mais informações sobre comandos e sessões remotas, consulte [about_Remote](about_Remote.md) e [about_PSSessions](about_PSSessions.md).</span><span class="sxs-lookup"><span data-stu-id="53fa8-442">For more information about remote commands and sessions, see [about_Remote](about_Remote.md) and [about_PSSessions](about_PSSessions.md).</span></span>

<span data-ttu-id="53fa8-443">Para alterar o valor da `$PSSessionOption` variável de preferência, use o `New-PSSessionOption` cmdlet para criar um objeto **PSSessionOption** com os valores de opção que você preferir.</span><span class="sxs-lookup"><span data-stu-id="53fa8-443">To change the value of the `$PSSessionOption` preference variable, use the `New-PSSessionOption` cmdlet to create a **PSSessionOption** object with the option values you prefer.</span></span> <span data-ttu-id="53fa8-444">Salve a saída em uma variável chamada `$PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-444">Save the output in a variable called `$PSSessionOption`.</span></span>

```powershell
$PSSessionOption = New-PSSessionOption -NoCompression
```

<span data-ttu-id="53fa8-445">Para usar a `$PSSessionOption` variável de preferência em cada sessão do PowerShell, adicione um `New-PSSessionOption` comando que cria a `$PSSessionOption` variável ao seu perfil do PowerShell.</span><span class="sxs-lookup"><span data-stu-id="53fa8-445">To use the `$PSSessionOption` preference variable in every PowerShell session, add a `New-PSSessionOption` command that creates the `$PSSessionOption` variable to your PowerShell profile.</span></span> <span data-ttu-id="53fa8-446">Para obter mais informações, consulte [about_Profiles](about_Profiles.md).</span><span class="sxs-lookup"><span data-stu-id="53fa8-446">For more information, see [about_Profiles](about_Profiles.md).</span></span>

<span data-ttu-id="53fa8-447">Você pode definir opções personalizadas para uma sessão remota específica.</span><span class="sxs-lookup"><span data-stu-id="53fa8-447">You can set custom options for a particular remote session.</span></span> <span data-ttu-id="53fa8-448">As opções definidas têm precedência sobre os padrões do sistema e o valor da variável de `$PSSessionOption` preferência.</span><span class="sxs-lookup"><span data-stu-id="53fa8-448">The options that you set take precedence over the system defaults and the value of the `$PSSessionOption` preference variable.</span></span>

<span data-ttu-id="53fa8-449">Para definir opções de sessão personalizadas, use o `New-PSSessionOption` cmdlet para criar um objeto **PSSessionOption** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-449">To set custom session options, use the `New-PSSessionOption` cmdlet to create a **PSSessionOption** object.</span></span> <span data-ttu-id="53fa8-450">Em seguida, use o objeto **PSSessionOption** como o valor do parâmetro **SessionOption** em cmdlets que criam uma sessão, como `New-PSSession` , `Enter-PSSession` e `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-450">Then, use the **PSSessionOption** object as the value of the **SessionOption** parameter in cmdlets that create a session, such as `New-PSSession`, `Enter-PSSession`, and `Invoke-Command`.</span></span>

### <a name="transcript"></a><span data-ttu-id="53fa8-451">$Transcript</span><span class="sxs-lookup"><span data-stu-id="53fa8-451">$Transcript</span></span>

<span data-ttu-id="53fa8-452">Usado pelo `Start-Transcript` para especificar o nome e o local do arquivo de transcrição.</span><span class="sxs-lookup"><span data-stu-id="53fa8-452">Used by `Start-Transcript` to specify the name and location of the transcript file.</span></span> <span data-ttu-id="53fa8-453">Se você não especificar um valor para o parâmetro **path** , o `Start-Transcript` usará o caminho no valor da `$Transcript` variável global.</span><span class="sxs-lookup"><span data-stu-id="53fa8-453">If you do not specify a value for the **Path** parameter, `Start-Transcript` uses the path in the value of the `$Transcript` global variable.</span></span> <span data-ttu-id="53fa8-454">Se você não tiver criado essa variável, `Start-Transcript` o armazenará as transcrições no `$Home\My Documents` diretório como `\PowerShell_transcript.<time-stamp>.txt` arquivos.</span><span class="sxs-lookup"><span data-stu-id="53fa8-454">If you have not created this variable, `Start-Transcript` stores the transcripts in the `$Home\My Documents` directory as `\PowerShell_transcript.<time-stamp>.txt` files.</span></span>

### <a name="verbosepreference"></a><span data-ttu-id="53fa8-455">\$VerbosePreference</span><span class="sxs-lookup"><span data-stu-id="53fa8-455">\$VerbosePreference</span></span>

<span data-ttu-id="53fa8-456">Determina como o PowerShell responde a mensagens detalhadas geradas por um script, um cmdlet ou um provedor, como as mensagens geradas pelo cmdlet [Write-Verbose](xref:Microsoft.PowerShell.Utility.Write-Verbose) .</span><span class="sxs-lookup"><span data-stu-id="53fa8-456">Determines how PowerShell responds to verbose messages generated by a script, cmdlet, or provider, such as the messages generated by the [Write-Verbose](xref:Microsoft.PowerShell.Utility.Write-Verbose) cmdlet.</span></span>
<span data-ttu-id="53fa8-457">As mensagens detalhadas descrevem as ações executadas para executar um comando.</span><span class="sxs-lookup"><span data-stu-id="53fa8-457">Verbose messages describe the actions performed to execute a command.</span></span>

<span data-ttu-id="53fa8-458">Por padrão, as mensagens detalhadas não são exibidas, mas você pode alterar esse comportamento alterando o valor de `$VerbosePreference` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-458">By default, verbose messages aren't displayed, but you can change this behavior by changing the value of `$VerbosePreference`.</span></span>

<span data-ttu-id="53fa8-459">Você pode usar o parâmetro comum **detalhado** de um cmdlet para exibir ou ocultar as mensagens detalhadas de um comando específico.</span><span class="sxs-lookup"><span data-stu-id="53fa8-459">You can use the **Verbose** common parameter of a cmdlet to display or hide the verbose messages for a specific command.</span></span> <span data-ttu-id="53fa8-460">Para obter mais informações, confira [about_CommonParameters](about_CommonParameters.md).</span><span class="sxs-lookup"><span data-stu-id="53fa8-460">For more information, see [about_CommonParameters](about_CommonParameters.md).</span></span>

<span data-ttu-id="53fa8-461">Os valores válidos são os seguintes:</span><span class="sxs-lookup"><span data-stu-id="53fa8-461">The valid values are as follows:</span></span>

- <span data-ttu-id="53fa8-462">**Parar** : exibe a mensagem detalhada e uma mensagem de erro e, em seguida, interrompe a execução.</span><span class="sxs-lookup"><span data-stu-id="53fa8-462">**Stop** : Displays the verbose message and an error message and then stops executing.</span></span>
- <span data-ttu-id="53fa8-463">**Consultar** : exibe a mensagem detalhada e, em seguida, exibe um prompt que pergunta se você deseja continuar.</span><span class="sxs-lookup"><span data-stu-id="53fa8-463">**Inquire** : Displays the verbose message and then displays a prompt that asks you whether you want to continue.</span></span>
- <span data-ttu-id="53fa8-464">**Continuar** : exibe a mensagem detalhada e continua com a execução.</span><span class="sxs-lookup"><span data-stu-id="53fa8-464">**Continue** : Displays the verbose message and then continues with execution.</span></span>
- <span data-ttu-id="53fa8-465">**SilentlyContinue** : (padrão) não exibe a mensagem detalhada.</span><span class="sxs-lookup"><span data-stu-id="53fa8-465">**SilentlyContinue** : (Default) Doesn't display the verbose message.</span></span> <span data-ttu-id="53fa8-466">Continua em execução.</span><span class="sxs-lookup"><span data-stu-id="53fa8-466">Continues executing.</span></span>

#### <a name="examples"></a><span data-ttu-id="53fa8-467">Exemplos</span><span class="sxs-lookup"><span data-stu-id="53fa8-467">Examples</span></span>

<span data-ttu-id="53fa8-468">Esses exemplos mostram o efeito dos diferentes valores de `$VerbosePreference` e o parâmetro **Verbose** para substituir o valor de preferência.</span><span class="sxs-lookup"><span data-stu-id="53fa8-468">These examples show the effect of the different values of `$VerbosePreference` and the **Verbose** parameter to override the preference value.</span></span>

<span data-ttu-id="53fa8-469">Este exemplo mostra o efeito do valor **SilentlyContinue** , que é o padrão.</span><span class="sxs-lookup"><span data-stu-id="53fa8-469">This example shows the effect of the **SilentlyContinue** value, that is the default.</span></span> <span data-ttu-id="53fa8-470">O comando usa o parâmetro **Message** , mas não grava uma mensagem no console do PowerShell.</span><span class="sxs-lookup"><span data-stu-id="53fa8-470">The command uses the **Message** parameter, but doesn't write a message to the PowerShell console.</span></span>

```powershell
Write-Verbose -Message "Verbose message test."
```

<span data-ttu-id="53fa8-471">Quando o parâmetro **Verbose** é usado, a mensagem é gravada.</span><span class="sxs-lookup"><span data-stu-id="53fa8-471">When the **Verbose** parameter is used, the message is written.</span></span>
...
<span data-ttu-id="53fa8-472">Este exemplo mostra o efeito do valor de **continuação** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-472">This example shows the effect of the **Continue** value.</span></span> <span data-ttu-id="53fa8-473">A `$VerbosePreference` variável é definida como **continuar** e a mensagem é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-473">The `$VerbosePreference` variable is set to **Continue** and the message is displayed.</span></span>
...
<span data-ttu-id="53fa8-474">Este exemplo usa o parâmetro **Verbose** com um valor `$false` que substitui o valor **continue** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-474">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Continue** value.</span></span> <span data-ttu-id="53fa8-475">A mensagem não é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-475">The message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

<span data-ttu-id="53fa8-476">Este exemplo mostra o efeito do valor de **parada** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-476">This example shows the effect of the **Stop** value.</span></span> <span data-ttu-id="53fa8-477">A `$VerbosePreference` variável é definida como **parar** e a mensagem é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-477">The `$VerbosePreference` variable is set to **Stop** and the message is displayed.</span></span> <span data-ttu-id="53fa8-478">O comando é interrompido.</span><span class="sxs-lookup"><span data-stu-id="53fa8-478">The command is stopped.</span></span>
...
<span data-ttu-id="53fa8-479">Este exemplo usa o parâmetro **Verbose** com um valor `$false` que substitui o valor de **parada** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-479">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Stop** value.</span></span> <span data-ttu-id="53fa8-480">A mensagem não é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-480">The message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

<span data-ttu-id="53fa8-481">Este exemplo mostra o efeito do valor de **consulta** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-481">This example shows the effect of the **Inquire** value.</span></span> <span data-ttu-id="53fa8-482">A `$VerbosePreference` variável é definida como **inquire** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-482">The `$VerbosePreference` variable is set to **Inquire** .</span></span> <span data-ttu-id="53fa8-483">A mensagem é exibida e a confirmação do usuário é solicitada.</span><span class="sxs-lookup"><span data-stu-id="53fa8-483">The message is displayed and the user is prompted for confirmation.</span></span>
...
<span data-ttu-id="53fa8-484">Este exemplo usa o parâmetro **Verbose** com um valor `$false` que substitui o valor de **consulta** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-484">This example uses the **Verbose** parameter with a value of `$false` that overrides the **Inquire** value.</span></span> <span data-ttu-id="53fa8-485">O usuário não é solicitado e a mensagem não é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-485">The user isn't prompted and the message isn't displayed.</span></span>

```powershell
Write-Verbose -Message "Verbose message test." -Verbose:$false
```

### <a name="warningpreference"></a><span data-ttu-id="53fa8-486">\$WarningPreference</span><span class="sxs-lookup"><span data-stu-id="53fa8-486">\$WarningPreference</span></span>

<span data-ttu-id="53fa8-487">Determina como o PowerShell responde a mensagens de aviso geradas por um script, um cmdlet ou um provedor, como as mensagens geradas pelo cmdlet [Write-Warning](xref:Microsoft.PowerShell.Utility.Write-Warning) .</span><span class="sxs-lookup"><span data-stu-id="53fa8-487">Determines how PowerShell responds to warning messages generated by a script, cmdlet, or provider, such as the messages generated by the [Write-Warning](xref:Microsoft.PowerShell.Utility.Write-Warning) cmdlet.</span></span>

<span data-ttu-id="53fa8-488">Por padrão, as mensagens de aviso são exibidas e a execução continua, mas você pode alterar esse comportamento alterando o valor de `$WarningPreference` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-488">By default, warning messages are displayed and execution continues, but you can change this behavior by changing the value of `$WarningPreference`.</span></span>

<span data-ttu-id="53fa8-489">Você pode usar o parâmetro de **aviso** comum de um cmdlet para determinar como o PowerShell responde a avisos de um comando específico.</span><span class="sxs-lookup"><span data-stu-id="53fa8-489">You can use the **WarningAction** common parameter of a cmdlet to determine how PowerShell responds to warnings from a particular command.</span></span> <span data-ttu-id="53fa8-490">Para obter mais informações, confira [about_CommonParameters](about_CommonParameters.md).</span><span class="sxs-lookup"><span data-stu-id="53fa8-490">For more information, see [about_CommonParameters](about_CommonParameters.md).</span></span>

<span data-ttu-id="53fa8-491">Os valores válidos são os seguintes:</span><span class="sxs-lookup"><span data-stu-id="53fa8-491">The valid values are as follows:</span></span>

- <span data-ttu-id="53fa8-492">**Parar** : exibe a mensagem de aviso e uma mensagem de erro e, em seguida, interrompe a execução.</span><span class="sxs-lookup"><span data-stu-id="53fa8-492">**Stop** : Displays the warning message and an error message and then stops executing.</span></span>
- <span data-ttu-id="53fa8-493">**Consultar** : exibe a mensagem de aviso e solicita permissão para continuar.</span><span class="sxs-lookup"><span data-stu-id="53fa8-493">**Inquire** : Displays the warning message and then prompts for permission to continue.</span></span>
- <span data-ttu-id="53fa8-494">**Continuar** : (padrão) exibe a mensagem de aviso e continua em execução.</span><span class="sxs-lookup"><span data-stu-id="53fa8-494">**Continue** : (Default) Displays the warning message and then continues executing.</span></span>
- <span data-ttu-id="53fa8-495">**SilentlyContinue** : não exibe a mensagem de aviso.</span><span class="sxs-lookup"><span data-stu-id="53fa8-495">**SilentlyContinue** : Doesn't display the warning message.</span></span> <span data-ttu-id="53fa8-496">Continua em execução.</span><span class="sxs-lookup"><span data-stu-id="53fa8-496">Continues executing.</span></span>

#### <a name="examples"></a><span data-ttu-id="53fa8-497">Exemplos</span><span class="sxs-lookup"><span data-stu-id="53fa8-497">Examples</span></span>

<span data-ttu-id="53fa8-498">Esses exemplos mostram o efeito dos diferentes valores de `$WarningPreference` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-498">These examples show the effect of the different values of `$WarningPreference`.</span></span>
<span data-ttu-id="53fa8-499">O parâmetro **WarningAction** substitui o valor de preferência.</span><span class="sxs-lookup"><span data-stu-id="53fa8-499">The **WarningAction** parameter overrides the preference value.</span></span>

<span data-ttu-id="53fa8-500">Este exemplo mostra o efeito do valor padrão, **continue** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-500">This example shows the effect of the default value, **Continue** .</span></span>
...
<span data-ttu-id="53fa8-501">Este exemplo usa o parâmetro **WarningAction** com o valor **SilentlyContinue** para suprimir o aviso.</span><span class="sxs-lookup"><span data-stu-id="53fa8-501">This example uses the **WarningAction** parameter with the value **SilentlyContinue** to suppress the warning.</span></span> <span data-ttu-id="53fa8-502">A mensagem não é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-502">The message isn't displayed.</span></span>
...
<span data-ttu-id="53fa8-503">Este exemplo altera a `$WarningPreference` variável para o valor **SilentlyContinue** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-503">This example changes the `$WarningPreference` variable to the **SilentlyContinue** value.</span></span> <span data-ttu-id="53fa8-504">A mensagem não é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-504">The message isn't displayed.</span></span>
...
<span data-ttu-id="53fa8-505">Este exemplo usa o parâmetro **WarningAction** para parar quando um aviso é gerado.</span><span class="sxs-lookup"><span data-stu-id="53fa8-505">This example uses the **WarningAction** parameter to stop when a warning is generated.</span></span>
...
<span data-ttu-id="53fa8-506">Este exemplo altera a `$WarningPreference` variável para o valor de **consulta** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-506">This example changes the `$WarningPreference` variable to the **Inquire** value.</span></span> <span data-ttu-id="53fa8-507">A confirmação do usuário é solicitada.</span><span class="sxs-lookup"><span data-stu-id="53fa8-507">The user is prompted for confirmation.</span></span>
...
<span data-ttu-id="53fa8-508">Este exemplo usa o parâmetro **WarningAction** com o valor **SilentlyContinue** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-508">This example uses the **WarningAction** parameter with the value **SilentlyContinue** .</span></span> <span data-ttu-id="53fa8-509">O comando continua a ser executado e nenhuma mensagem é exibida.</span><span class="sxs-lookup"><span data-stu-id="53fa8-509">The command continues to execute and no message is displayed.</span></span>
...
<span data-ttu-id="53fa8-510">Este exemplo altera o `$WarningPreference` valor para **parar** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-510">This example changes the `$WarningPreference` value to **Stop** .</span></span>
...
<span data-ttu-id="53fa8-511">Este exemplo usa o **avisoaction** com o valor de **consulta** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-511">This example uses the **WarningAction** with the **Inquire** value.</span></span> <span data-ttu-id="53fa8-512">O usuário receberá uma solicitação quando ocorrer um aviso.</span><span class="sxs-lookup"><span data-stu-id="53fa8-512">The user is prompted when a warning occurs.</span></span>
...
### <a name="whatifpreference"></a><span data-ttu-id="53fa8-513">\$WhatIfPreference</span><span class="sxs-lookup"><span data-stu-id="53fa8-513">\$WhatIfPreference</span></span>

<span data-ttu-id="53fa8-514">Determina se o **WhatIf** é habilitado automaticamente para cada comando que dá suporte a ele.</span><span class="sxs-lookup"><span data-stu-id="53fa8-514">Determines whether **WhatIf** is automatically enabled for every command that supports it.</span></span> <span data-ttu-id="53fa8-515">Quando o **WhatIf** é habilitado, o cmdlet relata o efeito esperado do comando, mas não executa o comando.</span><span class="sxs-lookup"><span data-stu-id="53fa8-515">When **WhatIf** is enabled, the cmdlet reports the expected effect of the command, but doesn't execute the command.</span></span>

<span data-ttu-id="53fa8-516">Os valores válidos são os seguintes:</span><span class="sxs-lookup"><span data-stu-id="53fa8-516">The valid values are as follows:</span></span>

- <span data-ttu-id="53fa8-517">**False** ( **0** , não habilitado): (padrão) **WhatIf** não é habilitado automaticamente.</span><span class="sxs-lookup"><span data-stu-id="53fa8-517">**False** ( **0** , not enabled): (Default) **WhatIf** isn't automatically enabled.</span></span> <span data-ttu-id="53fa8-518">Para habilitá-lo manualmente, use o parâmetro **WhatIf** do cmdlet.</span><span class="sxs-lookup"><span data-stu-id="53fa8-518">To enable it manually, use the cmdlet's **WhatIf** parameter.</span></span>
- <span data-ttu-id="53fa8-519">**True** ( **1** , Enabled): **WhatIf** é habilitado automaticamente em qualquer comando que ofereça suporte a ele.</span><span class="sxs-lookup"><span data-stu-id="53fa8-519">**True** ( **1** , enabled): **WhatIf** is automatically enabled on any command that supports it.</span></span> <span data-ttu-id="53fa8-520">Os usuários podem usar o parâmetro **WhatIf** com um valor de **false** para desabilitá-lo manualmente, como `-WhatIf:$false` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-520">Users can use the **WhatIf** parameter with a value of **False** to disable it manually, such as `-WhatIf:$false`.</span></span>

#### <a name="examples"></a><span data-ttu-id="53fa8-521">Exemplos</span><span class="sxs-lookup"><span data-stu-id="53fa8-521">Examples</span></span>

<span data-ttu-id="53fa8-522">Esses exemplos mostram o efeito dos diferentes valores de `$WhatIfPreference` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-522">These examples show the effect of the different values of `$WhatIfPreference`.</span></span>
<span data-ttu-id="53fa8-523">Eles mostram como usar o parâmetro **WhatIf** para substituir o valor de preferência para um comando específico.</span><span class="sxs-lookup"><span data-stu-id="53fa8-523">They show how to use the **WhatIf** parameter to override the preference value for a specific command.</span></span>

<span data-ttu-id="53fa8-524">Este exemplo mostra o efeito da `$WhatIfPreference` variável definida como o valor padrão, **false** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-524">This example shows the effect of the `$WhatIfPreference` variable set to the default value, **False** .</span></span> <span data-ttu-id="53fa8-525">Use `Get-ChildItem` para verificar se o arquivo existe.</span><span class="sxs-lookup"><span data-stu-id="53fa8-525">Use `Get-ChildItem` to verify that the file exists.</span></span>
<span data-ttu-id="53fa8-526">`Remove-Item` exclui o arquivo.</span><span class="sxs-lookup"><span data-stu-id="53fa8-526">`Remove-Item` deletes the file.</span></span> <span data-ttu-id="53fa8-527">Depois que o arquivo for excluído, você poderá verificar a exclusão com `Get-ChildItem` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-527">After the file is deleted, you can verify the deletion with `Get-ChildItem`.</span></span>
...
<span data-ttu-id="53fa8-528">Este exemplo mostra o efeito de usar o parâmetro **WhatIf** quando o valor de `$WhatIfPreference` é **false** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-528">This example shows the effect of using the **WhatIf** parameter when the value of `$WhatIfPreference` is **False** .</span></span>

<span data-ttu-id="53fa8-529">Verifique se o arquivo existe.</span><span class="sxs-lookup"><span data-stu-id="53fa8-529">Verify that the file exists.</span></span>
...
<span data-ttu-id="53fa8-530">Use o parâmetro **WhatIf** para determinar o resultado da tentativa de excluir o arquivo.</span><span class="sxs-lookup"><span data-stu-id="53fa8-530">Use the **WhatIf** parameter to determine the result of attempting to delete the file.</span></span>
...
<span data-ttu-id="53fa8-531">Verifique se o arquivo não foi excluído.</span><span class="sxs-lookup"><span data-stu-id="53fa8-531">Verify that the file wasn't deleted.</span></span>
...
<span data-ttu-id="53fa8-532">Este exemplo mostra o efeito da `$WhatIfPreference` variável definida como o valor, **true** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-532">This example shows the effect of the `$WhatIfPreference` variable set to the value, **True** .</span></span> <span data-ttu-id="53fa8-533">Quando você usa `Remove-Item` para excluir um arquivo, o caminho do arquivo é exibido, mas o arquivo não é excluído.</span><span class="sxs-lookup"><span data-stu-id="53fa8-533">When you use `Remove-Item` to delete a file, the file's path is displayed, but the file isn't deleted.</span></span>

<span data-ttu-id="53fa8-534">Tentativa de excluir um arquivo.</span><span class="sxs-lookup"><span data-stu-id="53fa8-534">Attempt to delete a file.</span></span> <span data-ttu-id="53fa8-535">Uma mensagem é exibida sobre o que aconteceria se `Remove-Item` fosse executado, mas o arquivo não é excluído.</span><span class="sxs-lookup"><span data-stu-id="53fa8-535">A message is displayed about what would happen if `Remove-Item` was run, but the file isn't deleted.</span></span>
...
<span data-ttu-id="53fa8-536">Use `Get-ChildItem` para verificar se o arquivo não foi excluído.</span><span class="sxs-lookup"><span data-stu-id="53fa8-536">Use `Get-ChildItem` to verify that the file wasn't deleted.</span></span>
...
<span data-ttu-id="53fa8-537">Este exemplo mostra como excluir um arquivo quando o valor de `$WhatIfPreference` for **true** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-537">This example shows how to delete a file when the value of `$WhatIfPreference` is **True** .</span></span> <span data-ttu-id="53fa8-538">Ele usa o parâmetro **WhatIf** com um valor de `$false` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-538">It uses the **WhatIf** parameter with a value of `$false`.</span></span> <span data-ttu-id="53fa8-539">Use `Get-ChildItem` para verificar se o arquivo foi excluído.</span><span class="sxs-lookup"><span data-stu-id="53fa8-539">Use `Get-ChildItem` to verify the file was deleted.</span></span>
...
<span data-ttu-id="53fa8-540">Veja a seguir exemplos do `Get-Process` cmdlet que não dá suporte a **WhatIf** e `Stop-Process` que oferece suporte a **WhatIf** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-540">The following are examples of the `Get-Process` cmdlet that doesn't support **WhatIf** and `Stop-Process` that does support **WhatIf** .</span></span> <span data-ttu-id="53fa8-541">O `$WhatIfPreference` valor da variável é **true** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-541">The `$WhatIfPreference` variable's value is **True** .</span></span>

<span data-ttu-id="53fa8-542">`Get-Process` Não dá suporte a **WhatIf** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-542">`Get-Process` doesn't support **WhatIf** .</span></span> <span data-ttu-id="53fa8-543">Quando o comando é executado, ele exibe o processo **Winword** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-543">When the command is executed, it displays the **Winword** process.</span></span>
...
<span data-ttu-id="53fa8-544">`Stop-Process` o oferece suporte a **WhatIf** .</span><span class="sxs-lookup"><span data-stu-id="53fa8-544">`Stop-Process` does support **WhatIf** .</span></span> <span data-ttu-id="53fa8-545">O processo de **Winword** não está parado.</span><span class="sxs-lookup"><span data-stu-id="53fa8-545">The **Winword** process isn't stopped.</span></span>
...
<span data-ttu-id="53fa8-546">Você pode substituir o `Stop-Process` comportamento de **WhatIf** usando o parâmetro **WhatIf** com um valor de `$false` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-546">You can override the `Stop-Process` **WhatIf** behavior by using the **WhatIf** parameter with a value of `$false`.</span></span> <span data-ttu-id="53fa8-547">O processo de **Winword** foi interrompido.</span><span class="sxs-lookup"><span data-stu-id="53fa8-547">The **Winword** process is stopped.</span></span>

```powershell
Stop-Process -Name Winword -WhatIf:$false
```

<span data-ttu-id="53fa8-548">Para verificar se o processo de **Winword** foi interrompido, use `Get-Process` .</span><span class="sxs-lookup"><span data-stu-id="53fa8-548">To verify that the **Winword** process was stopped, use `Get-Process`.</span></span>
...
## <a name="see-also"></a><span data-ttu-id="53fa8-549">Confira também</span><span class="sxs-lookup"><span data-stu-id="53fa8-549">See also</span></span>

[<span data-ttu-id="53fa8-550">about_Automatic_Variables</span><span class="sxs-lookup"><span data-stu-id="53fa8-550">about_Automatic_Variables</span></span>](about_Automatic_Variables.md)

[<span data-ttu-id="53fa8-551">about_CommonParameters</span><span class="sxs-lookup"><span data-stu-id="53fa8-551">about_CommonParameters</span></span>](about_CommonParameters.md)

[<span data-ttu-id="53fa8-552">about_Environment_Variables</span><span class="sxs-lookup"><span data-stu-id="53fa8-552">about_Environment_Variables</span></span>](about_Environment_Variables.md)

[<span data-ttu-id="53fa8-553">about_Profiles</span><span class="sxs-lookup"><span data-stu-id="53fa8-553">about_Profiles</span></span>](about_Profiles.md)

[<span data-ttu-id="53fa8-554">about_Remote</span><span class="sxs-lookup"><span data-stu-id="53fa8-554">about_Remote</span></span>](about_Remote.md)

[<span data-ttu-id="53fa8-555">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="53fa8-555">about_Scopes</span></span>](about_Scopes.md)

[<span data-ttu-id="53fa8-556">about_Variables</span><span class="sxs-lookup"><span data-stu-id="53fa8-556">about_Variables</span></span>](about_Variables.md)
