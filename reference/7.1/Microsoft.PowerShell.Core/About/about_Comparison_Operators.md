...
### <a name="replacement-with-a-script-block"></a>Substituição por um bloco de script
...
No bloco de script, use a `$_` variável automática para acessar o texto de entrada que está sendo substituído e outras informações úteis. O tipo de classe dessa variável é [System. Text. RegularExpressions. Match][2].

O exemplo a seguir substitui cada sequência de três dígitos pelos equivalentes de caractere. O bloco de script é executado para cada conjunto de três dígitos que precisam ser substituídos.
