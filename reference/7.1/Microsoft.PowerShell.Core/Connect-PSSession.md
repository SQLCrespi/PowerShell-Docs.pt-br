...
O cmdlet **Connect-PSSession** se reconecta a sessões do PowerShell gerenciadas pelo usuário ( **PSSessions** ) que foram desconectadas.
...
Por padrão, o PowerShell não redireciona conexões, mas você pode usar esse parâmetro para permitir que ele redirecione a conexão.
...
Para usar as portas padrão para comunicação remota do PowerShell, especifique a porta 5985 para HTTP ou 5986 para HTTPS.
...
Para configurar o ouvinte, digite os dois comandos a seguir no prompt do PowerShell:
...
WS-Management criptografa todo o conteúdo do PowerShell transmitido pela rede.
