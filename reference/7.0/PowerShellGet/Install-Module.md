...
O `Find-Module` usa o parâmetro **Name** para especificar o módulo **PowerShellGet** . Por padrão, a versão mais recente do módulo é baixada do repositório. O objeto é enviado ao pipeline para o `Install-Module` cmdlet. `Install-Module` instala o módulo para todos os usuários no `$env:ProgramFiles\PowerShell\Modules` .
...
`Install-Module` baixa e instala a versão mais recente do **PowerShellGet** no diretório do usuário atual, `$home\Documents\PowerShell\Modules` .
