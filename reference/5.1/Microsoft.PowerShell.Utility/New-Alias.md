...
O `New-Alias` cmdlet cria um novo alias na sessão atual do Windows PowerShell. Os aliases criados por meio `New-Alias` do não são salvos depois que você sai da sessão ou fecha o Windows PowerShell.
