...
O `Get-Process` cmdlet obtém objetos de **processo** . Os objetos de processo são enviados por meio do pipeline para o `Export-Csv` cmdlet. `Export-Csv` Converte os objetos de processo em uma série de cadeias de caracteres CSV.
O parâmetro **path** especifica que o arquivo de Processes.csv é salvo no diretório atual. O `Get-Content` cmdlet usa o parâmetro **Path** para exibir o arquivo localizado no diretório atual.
...
O `Export-CSV` cmdlet converte os objetos que você envia em uma série de cadeias de caracteres CSV e salva-os no arquivo de texto especificado. Você pode usar `Export-CSV` o para salvar objetos em um arquivo CSV e, em seguida, usar o `Import-Csv` cmdlet para criar objetos a partir do arquivo CSV.
