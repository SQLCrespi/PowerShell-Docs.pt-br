...
Quando você usa `powershell.exe` o com o parâmetro **File** , o `.ps1` próprio arquivo (script) deve incluir instruções para lidar com erros ou exceções que ocorrem durante a execução do script. Você só deve usar a `exit` instrução para indicar o status de pós-execução do script.

Qualquer número entre `[int]::MinValue` e `[int]::MaxValue` é permitido.
