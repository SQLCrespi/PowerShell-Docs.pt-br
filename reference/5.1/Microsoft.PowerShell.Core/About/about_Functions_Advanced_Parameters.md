...
Há um problema conhecido para usar coleções com **ValueFromRemainingArguments** , em que a coleção passada é tratada como um único elemento.

O exemplo a seguir demonstra esse problema conhecido. O parâmetro **restante** deve conter **um** no **índice 0** e **dois** no **índice 1** .
Em vez disso, ambos os elementos são combinados em uma única entidade.
...
> [!NOTE]
> Esse problema foi resolvido no PowerShell 6,2.
...
O atributo **ValidateRange** especifica um intervalo numérico para cada parâmetro ou valor de variável. O PowerShell gerará um erro se qualquer valor estiver fora desse intervalo. No exemplo a seguir, o valor do parâmetro de **tentativas** deve estar entre zero e dez.
