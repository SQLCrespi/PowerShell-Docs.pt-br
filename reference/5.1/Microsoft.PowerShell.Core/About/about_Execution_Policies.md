...
As políticas de execução do PowerShell são as seguintes:
...
Você pode usar o parâmetro **ExecutionPolicy** de **powershell.exe** para definir uma política de execução para uma nova sessão do PowerShell. A política afeta apenas a sessão atual e as sessões filho.

Para definir a política de execução para uma nova sessão, inicie o PowerShell na linha de comando, como **cmd.exe** ou do PowerShell e, em seguida, use o parâmetro **ExecutionPolicy** de **powershell.exe** para definir a política de execução.
...
- Política de execução: processo (ou `powershell.exe -ExecutionPolicy` )
...
Quando o PowerShell 5,1 é executado no Windows Server Core ou no Windows nano Server em determinadas condições, as políticas de execução podem falhar com o seguinte erro:
...
[PowerShell.exe Command-Line ajuda](about_PowerShell_exe.md)
