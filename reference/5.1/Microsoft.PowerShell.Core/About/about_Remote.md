...
Observação: para usar a comunicação remota do Windows PowerShell, os computadores locais e remotos devem ser configurados para comunicação remota. Para obter mais informações, consulte [about_Remote_Requirements](about_Remote_Requirements.md).
...
Como esses cmdlets não usam a comunicação remota do Windows PowerShell baseada no WS-Management, você pode usar o parâmetro ComputerName desses cmdlets em qualquer computador que esteja executando o Windows PowerShell. Os computadores não precisam ser configurados para comunicação remota do Windows PowerShell e os computadores não precisam atender aos requisitos do sistema para comunicação remota.
...
- Para obter informações sobre trabalhos em segundo plano do Windows PowerShell, consulte [about_Jobs](about_Jobs.md).
