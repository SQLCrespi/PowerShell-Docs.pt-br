...
Este exemplo mostra o efeito do `$DebugPreference` valor padrão da variável, **SilentlyContinue** . Por padrão, a `Write-Debug` mensagem de depuração do cmdlet não é exibida e o processamento continua. Quando o parâmetro de **depuração** é usado, ele substitui a preferência por um único comando. A confirmação do usuário é solicitada.
...
- **NormalView** : (padrão) uma exibição detalhada projetada para a maioria dos usuários. Consiste em uma descrição do erro e no nome do objeto envolvido no erro.
...
Este exemplo mostra como um erro aparece quando o valor de `$ErrorView` é o padrão, **NormalView** . `Get-ChildItem` é usado para localizar um arquivo não existente.
...
- **Suspender** : suspende automaticamente um trabalho de fluxo de trabalho depois `Write-Information` que um comando é executado, para permitir que os usuários vejam as mensagens antes de continuar. O fluxo de trabalho pode ser retomado a critério do usuário.
...
Determina quantos aliases são permitidos em uma sessão do PowerShell. O valor padrão é **4096** e deve ser suficiente para a maioria dos usos. Você pode se ajustar `$MaximumAliasCount` para atender às suas necessidades.

**Valores válidos** : 1024-32768 ( `Int32` )

**Padrão** : 4096

Para contar os aliases em seu sistema, digite:
...
Determina quantas unidades do PowerShell são permitidas em uma determinada sessão. Por exemplo, unidades do sistema de arquivos e armazenamentos de dados que são expostos por provedores do PowerShell e aparecem como unidades, como as `Alias:` `HKLM:` unidades e.
...
Determina quantos erros são salvos no histórico de erros da sessão.

**Valores válidos** : 256-32768 ( `Int32` )

**Padrão** : 256

Os objetos que representam cada erro retido são armazenados na `$Error` variável automática. `$Error` contém uma matriz de objetos de registro de erro. O erro mais recente é o primeiro objeto na matriz, `$Error[0]` .

Para contar os erros no seu sistema, use a `$Error` propriedade **Count** da matriz.

```powershell
$Error.count
```

Para exibir um erro específico, use a `[0]` notação de matriz para ver o erro mais recente.
...
O parâmetro **Force** substitui a formatação especial de objetos **ErrorRecord** e reverte para o formato convencional. Para exibir as propriedades do objeto **ErrorRecord** , digite o seguinte comando:

```powershell
$Error[0] | Format-List -Property * -Force
```

Neste exemplo, `$Error.Count` exibe o número de erros. Para excluir todos os erros do histórico de erros, use o `Clear` método da matriz de erros.
...
Para localizar todas as propriedades e métodos de uma matriz de erros, use o `Get-Member` cmdlet com seu parâmetro **InputObject** . Quando você usa o parâmetro **InputObject** , `Get-Member` o exibe as propriedades e os métodos da coleção.

```powershell
Get-Member -InputObject $Error
```

Quando você canaliza uma coleção de objetos para `Get-Member` , `Get-Member` o exibe as propriedades e os métodos dos objetos na coleção.
...
Para ver as funções em sua sessão, use a unidade do PowerShell `Function:` que é exposta pelo provedor do PowerShell `Function` . Para obter mais informações sobre o `Function` provedor, [about_Function_Provider](about_Function_Provider.md).
...
Determina quantas variáveis são permitidas em uma determinada sessão, incluindo variáveis automáticas, variáveis de preferência e as variáveis que você cria em comandos e scripts.

**Valores válidos** : 1024-32768 ( `Int32` )

**Padrão** : 4096

Para ver as variáveis em sua sessão, use o `Get-Variable` cmdlet e os recursos da unidade do PowerShell `Variable:` e o provedor do PowerShell `Variable` . Para obter informações, consulte [about_Variable_Provider](about_Variable_Provider.md).
