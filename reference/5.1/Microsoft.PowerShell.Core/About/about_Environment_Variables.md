...
O escopo do _processo_ contém as variáveis de ambiente disponíveis no processo atual ou na sessão do PowerShell. Essa lista de variáveis é herdada do processo pai e é construída a partir das variáveis nos escopos de _computador_ e de _usuário_ .
...
- PSModuleAnalysisCachePath
  ...
  O nome de arquivo padrão para o cache é `ModuleAnalysisCache` . Para alterar o local padrão do cache, defina a variável de ambiente antes de iniciar o PowerShell. As alterações nessa variável de ambiente afetam apenas os processos filho.
