---
description: Sobre tópicos abrangem um intervalo de conceitos sobre o PowerShell.
Download Help Link: https://Please-enter-FwLink-manually
Help Version: 6
keywords: powershell, cmdlet
Locale: en-US
Module Guid: 00000000-0000-0000-0000-000000000000
Module Name: About
ms.date: 02/25/2019
schema: 2.0.0
title: Sobre tópicos
ms.openlocfilehash: a315f5ee986dbcbf5016207c47a123999b0cbf4e
ms.sourcegitcommit: f874dc1d4236e06a3df195d179f59e0a7d9f8436
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/13/2020
ms.locfileid: "93196045"
---
# Sobre tópicos

## Descrição

Sobre tópicos abrangem um intervalo de conceitos sobre o PowerShell.

## Sobre tópicos

### [about_Alias_Provider](about_Alias_Provider.md)
Descreve os recursos do provedor de alias.

### [about_Aliases](about_Aliases.md)
Descreve como usar nomes alternativos para cmdlets e comandos no PowerShell.

### [about_Arithmetic_Operators](about_Arithmetic_Operators.md)
Descreve os operadores que executam aritmética no PowerShell.

### [about_Arrays](about_Arrays.md)
Descreve matrizes, que são estruturas de dados projetadas para armazenar coleções de itens.

### [about_Assignment_Operators](about_Assignment_Operators.md)
Descreve como usar operadores para atribuir valores a variáveis.

### [about_Automatic_Variables](about_Automatic_Variables.md)
Descreve as variáveis que armazenam informações de estado do PowerShell. Essas variáveis são criadas e mantidas pelo PowerShell.

### [about_Break](about_Break.md)
Descreve uma instrução que você pode usar para sair imediatamente das instruções Foreach, for, while, do ou switch.

### [about_CimSession](about_CimSession.md)
Descreve um objeto CimSession e a diferença entre sessões CIM e sessões do PowerShell.

### [about_Classes](about_Classes.md)
Descreve como você pode usar classes para criar seus próprios tipos personalizados.

### [about_Command_Precedence](about_Command_Precedence.md)
Descreve como o PowerShell determina qual comando executar.

### [about_Command_Syntax](about_Command_Syntax.md)
Descreve os diagramas de sintaxe usados no PowerShell.

### [about_Comment_Based_Help](about_Comment_Based_Help.md)
Descreve como escrever tópicos de ajuda baseados em comentários para funções e scripts.

### [about_CommonParameters](about_CommonParameters.md)
Descreve os parâmetros que podem ser usados com qualquer cmdlet.

### [about_Comparison_Operators](about_Comparison_Operators.md)
Descreve os operadores que comparam valores no PowerShell.

### [about_Continue](about_Continue.md)
Descreve como a `Continue` instrução retorna imediatamente o fluxo do programa para a parte superior de um loop de programa.

### [about_Core_Commands](about_Core_Commands.md)
Lista os cmdlets que foram projetados para uso com provedores do PowerShell.

### [about_Data_Sections](about_Data_Sections.md)
Explica as seções de dados, que isolam cadeias de caracteres de texto e outros dados somente leitura da lógica de script.

### [about_Debuggers](about_Debuggers.md)
Descreve o depurador do PowerShell.

### [about_desiredstateconfiguration](about_desiredstateconfiguration.md)
Fornece uma breve introdução ao recurso de configuração de estado desejado (DSC) do PowerShell.

### [about_Do](about_Do.md)
Executa uma lista de instruções uma ou mais vezes, sujeito a uma condição while ou until.

### [about_dsclogresource](about_dsclogresource.md)
Fornece uma breve introdução ao recurso de log de configuração de estado desejado (DSC) do PowerShell.

### [about_Enum](about_Enum.md)
A `enum` instrução é usada para declarar uma enumeração. Uma enumeração é um tipo distinto que consiste em um conjunto de rótulos nomeados chamado lista de enumeradores.

### [about_Environment_Provider](about_Environment_Provider.md)
Descreve os recursos do provedor de ambiente.

### [about_Environment_Variables](about_Environment_Variables.md)
Descreve como acessar variáveis de ambiente do Windows no PowerShell.

### [about_Eventlogs](about_Eventlogs.md)
O Windows PowerShell cria um log de eventos do Windows chamado "Windows PowerShell" para registrar eventos do Windows PowerShell.

### [about_Execution_Policies](about_Execution_Policies.md)
Descreve as políticas de execução do PowerShell e explica como gerenciá-las.

### [about_FileSystem_Provider](about_FileSystem_Provider.md)
Descreve os recursos do provedor FileSystem.

### [about_For](about_For.md)
Descreve um comando de linguagem que você pode usar para executar instruções com base em um teste condicional.

### [about_Foreach](about_Foreach.md)
Descreve um comando de linguagem que você pode usar para percorrer todos os itens em uma coleção de itens.

### [about_Format.ps1xml](about_Format.ps1xml.md)
Os `Format.ps1xml` arquivos no PowerShell definem a exibição padrão de objetos no console do PowerShell.

### [about_Function_Provider](about_Function_Provider.md)
Descreve os recursos do provedor de funções.

### [about_Functions](about_Functions.md)
Descreve como criar e usar funções no PowerShell.

### [about_Functions_Advanced](about_Functions_Advanced.md)
Apresenta funções avançadas que atuam de forma semelhante aos cmdlets.

### [about_Functions_Advanced_Methods](about_Functions_Advanced_Methods.md)
Descreve como as funções que especificam o `CmdletBinding` atributo podem usar os métodos e as propriedades que estão disponíveis para cmdlets compilados.

### [about_Functions_Advanced_Parameters](about_Functions_Advanced_Parameters.md)
Explica como adicionar parâmetros a funções avançadas.

### [about_Functions_CmdletBindingAttribute](about_Functions_CmdletBindingAttribute.md)
Descreve o atributo que faz uma função funcionar como um cmdlet compilado.

### [about_Functions_OutputTypeAttribute](about_Functions_OutputTypeAttribute.md)
Descreve um atributo que informa o tipo de objeto que a função retorna.

### [about_Group_Policy_Settings](about_Group_Policy_Settings.md)
Descreve as configurações de Política de Grupo do PowerShell.

### [about_Hash_Tables](about_Hash_Tables.md)
Descreve como criar, usar e classificar tabelas de hash no PowerShell.

### [about_hidden](about_hidden.md)
Descreve a palavra-chave Hidden, que oculta membros de classe dos resultados de Get-Member padrão.

### [about_History](about_History.md)
Descreve como obter e executar comandos no histórico de comandos.

### [about_If](about_If.md)
Descreve um comando de linguagem que você pode usar para executar listas de instruções com base nos resultados de um ou mais testes condicionais.

### [about_Job_Details](about_Job_Details.md)
Fornece detalhes sobre trabalhos em segundo plano em computadores locais e remotos.

### [about_Jobs](about_Jobs.md)
Fornece informações sobre como os trabalhos em segundo plano do PowerShell executam um comando ou expressão em segundo plano sem interagir com a sessão atual.

### [about_Join](about_Join.md)
Descreve como o operador de junção (-join) combina várias cadeias de caracteres em uma única cadeia.

### [about_Language_Keywords](about_Language_Keywords.md)
Descreve as palavras-chave na linguagem de script do PowerShell.

### [about_Language_Modes](about_Language_Modes.md)
Explica os modos de linguagem e seu efeito nas sessões do PowerShell.

### [about_Line_Editing](about_Line_Editing.md)
Descreve como editar comandos no prompt de comando do PowerShell.

### [about_locations](about_locations.md)
Descreve como acessar itens do local de trabalho no PowerShell.

### [about_Logging](about_Logging.md)
O PowerShell registra em log as operações internas do mecanismo, provedores e cmdlets.

### [about_logical_operators](about_logical_operators.md)
Descreve os operadores que conectam instruções no PowerShell.

### [about_Methods](about_Methods.md)
Descreve como usar métodos para executar ações em objetos no PowerShell.

### [about_Modules](about_Modules.md)
Explica como instalar, importar e usar módulos do PowerShell.

### [about_Object_Creation](about_Object_Creation.md)
Explica como criar objetos no PowerShell.

### [about_Objects](about_Objects.md)
Fornece informações essenciais sobre objetos no PowerShell.

### [about_Operator_Precedence](about_Operator_Precedence.md)
Lista os operadores do PowerShell em ordem de precedência.

### [about_Operators](about_Operators.md)
Descreve os operadores com suporte do PowerShell.

### [about_PackageManagement](about_PackageManagement.md)
PackageManagement é um agregador para gerenciadores de pacotes de software.

### [about_Parameters](about_Parameters.md)
Descreve como trabalhar com parâmetros de comando no PowerShell.

### [about_Parameters_Default_Values](about_Parameters_Default_Values.md)
Descreve como definir valores padrão personalizados para os parâmetros de cmdlets e funções avançadas.

### [about_Parsing](about_Parsing.md)
Descreve como o PowerShell analisa os comandos.

### [about_Path_Syntax](about_Path_Syntax.md)
Descreve os formatos de nome de caminho completo e relativo no PowerShell.

### [about_pipelines](about_pipelines.md)
Combinando comandos em pipelines no PowerShell.

### [about_PowerShell_exe](about_PowerShell_exe.md)
Explica como usar a ferramenta de linha de comando PowerShell.exe. Exibe a sintaxe e descreve as opções de linha de comando.

### [about_PowerShell_Ise_exe](about_PowerShell_Ise_exe.md)
Explica como usar a ferramenta de linha de comando PowerShell_Ise.exe.

### [about_Preference_Variables](about_Preference_Variables.md)
Variáveis que personalizam o comportamento do PowerShell.

### [about_Profiles](about_Profiles.md)
Descreve como criar e usar um perfil do PowerShell.

### [about_Prompts](about_Prompts.md)
Descreve a função de prompt e demonstra como criar uma função de prompt personalizada.

### [about_Properties](about_Properties.md)
Descreve como usar propriedades de objeto no PowerShell.

### [about_Providers](about_Providers.md)
Descreve como os provedores do PowerShell fornecem acesso a dados e componentes que, de outra forma, não seriam facilmente acessíveis na linha de comando.

### [about_psconsolehostreadline](about_psconsolehostreadline.md)
Explica como criar uma personalização de como o PowerShell lê a entrada no prompt do console.

### [about_PSSession_Details](about_PSSession_Details.md)
Fornece informações detalhadas sobre as sessões do PowerShell e a função que elas desempenham em comandos remotos.

### [about_PSSessions](about_PSSessions.md)
Descreve sessões do PowerShell (PSSessions) e explica como estabelecer uma conexão persistente com um computador remoto.

### [about_PSSnapins](about_PSSnapins.md)
Descreve os snap-ins do PowerShell e mostra como usá-los e gerenciá-los.

### [about_Quoting_Rules](about_Quoting_Rules.md)
Descreve regras para usar aspas simples e duplas no PowerShell.

### [about_Redirection](about_Redirection.md)
Explica como redirecionar a saída do PowerShell para arquivos de texto.

### [about_Ref](about_Ref.md)
Descreve como criar e usar uma variável de tipo de referência. Você pode usar variáveis de tipo de referência para permitir que uma função altere o valor de uma variável que é passada para ela.

### [about_Registry_Provider](about_Registry_Provider.md)
Descreve os recursos do provedor de registro.

### [about_Regular_Expressions](about_Regular_Expressions.md)
Descreve expressões regulares no PowerShell.

### [about_Remote](about_Remote.md)
Descreve como executar comandos remotos no PowerShell.

### [about_Remote_Disconnected_Sessions](about_Remote_Disconnected_Sessions.md)
Explica como desconectar e reconectar um PSSession.

### [about_Remote_FAQ](about_Remote_FAQ.md)
Contém perguntas e respostas sobre a execução de comandos remotos no PowerShell.

### [about_Remote_Jobs](about_Remote_Jobs.md)
Descreve como executar trabalhos em segundo plano em computadores remotos.

### [about_Remote_Output](about_Remote_Output.md)
Descreve como interpretar e formatar a saída de comandos remotos.

### [about_Remote_Requirements](about_Remote_Requirements.md)
Descreve os requisitos de sistema e os requisitos de configuração para executar comandos remotos no PowerShell.

### [about_Remote_Troubleshooting](about_Remote_Troubleshooting.md)
Descreve como solucionar problemas de operações remotas no PowerShell.

### [about_Remote_Variables](about_Remote_Variables.md)
Explica como usar variáveis locais e remotas em comandos remotos.

### [about_Requires](about_Requires.md)
Impede que um script seja executado sem os elementos necessários.

### [about_Reserved_Words](about_Reserved_Words.md)
Lista as palavras reservadas que não podem ser usadas como identificadores porque elas têm um significado especial no PowerShell.

### [about_Return](about_Return.md)
Sai do escopo atual, que pode ser uma função, um script ou um bloco de script.

### [about_Run_With_PowerShell](about_Run_With_PowerShell.md)
Explica como usar o recurso "executar com o PowerShell" para executar um script de uma unidade do sistema de arquivos.

### [about_Scopes](about_Scopes.md)
Explica o conceito de escopo no PowerShell e mostra como definir e alterar o escopo dos elementos.

### [about_Script_Blocks](about_Script_Blocks.md)
Define o que é um bloco de script e explica como usar blocos de script na linguagem de programação do PowerShell.

### [about_Script_Internationalization](about_Script_Internationalization.md)
Descreve os recursos de internacionalização de script que tornam mais fácil para os scripts exibir mensagens e instruções para os usuários em sua linguagem de interface do usuário.

### [about_Scripts](about_Scripts.md)
Descreve como executar e gravar scripts no PowerShell.

### [about_Session_Configuration_Files](about_Session_Configuration_Files.md)
Descreve os arquivos de configuração de sessão, que são usados em uma configuração de sessão (também conhecida como "ponto de extremidade") para definir o ambiente de sessões que usam a configuração de sessão.

### [about_Session_Configurations](about_Session_Configurations.md)
Descreve as configurações de sessão, que determinam os usuários que podem se conectar ao computador remotamente e os comandos que eles podem executar.

### [about_Signing](about_Signing.md)
Explica como assinar scripts para que eles estejam em conformidade com as políticas de execução do PowerShell.

### [about_simplified_syntax](about_simplified_syntax.md)
Descreve maneiras mais fáceis e mais naturais de scripts de filtros para coleções de objetos.

### [about_Special_Characters](about_Special_Characters.md)
Descreve os caracteres especiais que você pode usar para controlar como o PowerShell interpreta o próximo caractere em um comando ou parâmetro.

### [about_Splatting](about_Splatting.md)
Descreve como usar nivelamento para passar parâmetros para comandos no PowerShell.

### [about_Split](about_Split.md)
Explica como usar o operador split para dividir uma ou mais cadeias de caracteres em subcadeias.

### [about_Switch](about_Switch.md)
Explica como usar uma opção para manipular várias `If` instruções.

### [about_Throw](about_Throw.md)
Descreve a palavra-chave Throw, que gera um erro de terminação.

### [about_Transactions](about_Transactions.md)
Descreve como gerenciar operações transacionadas no PowerShell.

### [about_Trap](about_Trap.md)
Descreve uma palavra-chave que manipula um erro de encerramento.

### [about_Try_Catch_Finally](about_Try_Catch_Finally.md)
Descreve como usar os `Try` blocos, `Catch` e `Finally` para lidar com erros de encerramento.

### [about_Type_Operators](about_Type_Operators.md)
Descreve os operadores que funcionam com Microsoft .NET tipos de estrutura.

### [about_Types.ps1xml](about_Types.ps1xml.md)
Explica como usar Types.ps1arquivos XML para estender os tipos de objetos que são usados no PowerShell.

### [about_Updatable_Help](about_Updatable_Help.md)
Descreve o sistema de ajuda atualizável no PowerShell.

### [About_Using](About_Using.md)
Permite indicar quais namespaces são usados na sessão.

### [about_Variable_Provider](about_Variable_Provider.md)
Descreve os recursos do provedor de variáveis.

### [about_Variables](about_Variables.md)
Descreve como as variáveis armazenam valores que podem ser usados no PowerShell.

### [about_While](about_While.md)
Descreve uma instrução de linguagem que você pode usar para executar um bloco de comando com base nos resultados de um teste condicional.

### [about_Wildcards](about_Wildcards.md)
Descreve como usar caracteres curinga no PowerShell.

### [about_windows_powershell_5.1](about_windows_powershell_5.1.md)
Descreve os novos recursos que estão incluídos no PowerShell 5,1.

### [about_Windows_PowerShell_ISE](about_Windows_PowerShell_ISE.md)
Descreve os recursos e os requisitos de sistema do Ambiente de Script Integrado do Windows PowerShell (ISE).

### [about_Windows_RT](about_Windows_RT.md)
Explica as limitações do PowerShell 4,0 no Windows RT 8,1.

### [about_WMI](about_WMI.md)
O Instrumentação de Gerenciamento do Windows (WMI) usa o modelo CIM (CIM) para representar sistemas, aplicativos, redes, dispositivos e outros componentes gerenciáveis da empresa moderna.

### [about_WMI_Cmdlets](about_WMI_Cmdlets.md)
Fornece informações básicas sobre Instrumentação de Gerenciamento do Windows (WMI) e PowerShell.

### [about_WQL](about_WQL.md)
Descreve linguagem WQL (WQL), que pode ser usado para obter objetos WMI no PowerShell.
