...
### Exemplo 1: alterar o estado de apartment do thread

```
PS C:\> Set-PSSessionConfiguration -Name "MaintenanceShell" -ThreadApartmentState STA
```

Esse comando altera o estado de apartment do thread na configuração MaintenanceShell para STA.
A alteração é eficaz quando você reinicia o serviço **WinRM** .
...
### Exemplo 4: exibir resultados de diferentes maneiras
...
Especifica a configuração de estado de apartamento para os threads na sessão.
Os valores aceitáveis para esse parâmetro são: STA, MTA e Unknown.
