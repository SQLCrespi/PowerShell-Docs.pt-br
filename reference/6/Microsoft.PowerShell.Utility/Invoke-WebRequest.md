...
> [!NOTE]
> Em plataformas não Windows, talvez não seja possível fornecer `'Tls, Tls12'` uma opção.
...
Para obter mais informações sobre o tipo de objeto **Microsoft. PowerShell. Commands. BasicHtmlWebResponseObject** , consulte [BasicHtmlWebResponseObject](/dotnet/api/microsoft.powershell.commands.basichtmlwebresponseobject).

Para obter mais informações sobre como o .NET fornece serviços de proxy para o PowerShell, consulte [acessando a Internet por meio de um proxy](/dotnet/framework/network-programming/accessing-the-internet-through-a-proxy).
