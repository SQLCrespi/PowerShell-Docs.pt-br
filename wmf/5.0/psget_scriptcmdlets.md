...
---
# <a name="powershellget-cmdlets-for-script-management"></a><span data-ttu-id="41f54-102">Cmdlets do PowerShellGet para Gerenciamento de Script</span><span class="sxs-lookup"><span data-stu-id="41f54-102">PowerShellGet Cmdlets for Script Management</span></span>

## <a name="find-script-cmdlet"></a><span data-ttu-id="41f54-103">Cmdlet Find-Script</span><span class="sxs-lookup"><span data-stu-id="41f54-103">Find-Script cmdlet</span></span>
<span data-ttu-id="41f54-104">O cmdlet Find-Script permite descobrir os arquivos de script com critérios de pesquisa diferentes, como nome, marca, filtro, nome do comando, intervalo de versão, a versão exata, todas as versões, incluindo suas dependências e de repositórios específicos ou de todos os repositórios.</span><span class="sxs-lookup"><span data-stu-id="41f54-104">Find-Script cmdlet lets you to discover the script files with different search criteria like name, tag, filter, command name, version range, exact version, all versions, including its dependencies and from specific or all registered repositories.</span></span>

<span data-ttu-id="41f54-105">Exemplo de uso:</span><span class="sxs-lookup"><span data-stu-id="41f54-105">Example usage:</span></span>
```powershell
...
## <a name="save-script-cmdlet"></a><span data-ttu-id="41f54-106">Cmdlet Save-Script</span><span class="sxs-lookup"><span data-stu-id="41f54-106">Save-Script cmdlet</span></span>
<span data-ttu-id="41f54-107">O cmdlet Save-Script permite examinar o arquivo de script salvando-o em um local especificado.</span><span class="sxs-lookup"><span data-stu-id="41f54-107">Save-Script cmdlet lets you to review the script file by saving it to a specified location.</span></span>
```powershell
...
## <a name="install-script-and-get-installedscript-cmdlets"></a><span data-ttu-id="41f54-108">Cmdlets Install-Script e Get-InstalledScript</span><span class="sxs-lookup"><span data-stu-id="41f54-108">Install-Script and Get-InstalledScript cmdlets</span></span>
<span data-ttu-id="41f54-109">O cmdlet Install-Script permite instalar um arquivo de script específico junto com suas dependências no escopo especificado.</span><span class="sxs-lookup"><span data-stu-id="41f54-109">Install-Script cmdlet lets you to install a specific script file along with its dependencies to the specified scope.</span></span> <span data-ttu-id="41f54-110">Por padrão, os scripts são instalados no escopo AllUsers.</span><span class="sxs-lookup"><span data-stu-id="41f54-110">By default, scripts are installed to the AllUsers scope.</span></span> <span data-ttu-id="41f54-111">O cmdlet Get-InstalledScript permite obter a lista de arquivos de script que foram instalados com o cmdlet Install-Script.</span><span class="sxs-lookup"><span data-stu-id="41f54-111">Get-InstalledScript cmdlet lets you to get the list of script files which were installed using Install-Script cmdlet.</span></span>

<span data-ttu-id="41f54-112">Observação de uso: para permitir o gerenciamento e a localização dos scripts quando eles são instalados, o Install-Script cria uma pasta padrão para armazenar scripts em $home\Documents\WindowsPowerShell\Scripts e adiciona essa pasta ao seu ambiente PATH.</span><span class="sxs-lookup"><span data-stu-id="41f54-112">Use note: To allow management and locating of scripts once they are installed, Install-script will create a default folder for storing scripts at $home\Documents\WindowsPowerShell\Scripts, and add that folder to your PATH environment.</span></span> <span data-ttu-id="41f54-113">Caso a modificação do caminho seja uma preocupação, use Save-Script em vez de Install-Script.</span><span class="sxs-lookup"><span data-stu-id="41f54-113">If modifying the path is a concern, use Save-Script instead of Install-Script.</span></span> <span data-ttu-id="41f54-114">Get-InstalledScripts e Uninstall-Script só podem funcionar com scripts colocados no sistema usando Install-Script.</span><span class="sxs-lookup"><span data-stu-id="41f54-114">Get-InstalledScripts and Uninstall-Script can only work with scripts placed on the system using Install-Script.</span></span>
```powershell
...
<span data-ttu-id="41f54-115">Também é possível usar Get-Command –Name &lt;InstalledScriptFileName&gt; para obtê-lo.</span><span class="sxs-lookup"><span data-stu-id="41f54-115">You can also use Get-Command –Name &lt;InstalledScriptFileName&gt; to get it.</span></span> <span data-ttu-id="41f54-116">Dois locais de instalação são adicionados à variável de ambiente PATH na primeira utilização de um escopo especificado.</span><span class="sxs-lookup"><span data-stu-id="41f54-116">Two install locations are added to the PATH environment variable on first use of a specified scope.</span></span>
```powershell
...
## <a name="update-script-cmdlet"></a><span data-ttu-id="41f54-117">Cmdlet Update-Script</span><span class="sxs-lookup"><span data-stu-id="41f54-117">Update-Script cmdlet</span></span>
<span data-ttu-id="41f54-118">O cmdlet Update-Script permite a atualização in-loco dos arquivos de script que foram instalados usando o cmdlet Install-Script.</span><span class="sxs-lookup"><span data-stu-id="41f54-118">Update-Script cmdlet lets you to do in-place update of the script files which were installed using Install-Script cmdlet.</span></span>
```powershell
...
## <a name="uninstall-script-cmdlet"></a><span data-ttu-id="41f54-119">Cmdlet Uninstall-Script</span><span class="sxs-lookup"><span data-stu-id="41f54-119">Uninstall-Script cmdlet</span></span>
```powershell
...
## <a name="new-scriptfileinfo-and-test-scriptfileinfo-cmdlets"></a><span data-ttu-id="41f54-120">Cmdlets New-ScriptFileInfo e Test-ScriptFileInfo</span><span class="sxs-lookup"><span data-stu-id="41f54-120">New-ScriptFileInfo and Test-ScriptFileInfo cmdlets</span></span>
<span data-ttu-id="41f54-121">O cmdlet New-ScriptFileInfo permite criar um novo arquivo de script com metadados como Versão, Guid, Autor, Descrição, etc. O cmdlet Test-ScriptFileInfo permite validar e obter os metadados do arquivo de script.</span><span class="sxs-lookup"><span data-stu-id="41f54-121">New-ScriptFileInfo cmdlet lets you to create a new script file with metadata like Version, Guid, Author, and Description, etc. Test-ScriptFileInfo cmdlet lets you to validate and get the script file metadata.</span></span>
```powershell
...
## <a name="update-scriptfileinfo-cmdlet"></a><span data-ttu-id="41f54-122">Cmdlet Update-ScriptFileInfo</span><span class="sxs-lookup"><span data-stu-id="41f54-122">Update-ScriptFileInfo cmdlet</span></span>
<span data-ttu-id="41f54-123">O cmdlet Update-ScriptFileInfo permite atualizar os metadados do arquivo de script existente.</span><span class="sxs-lookup"><span data-stu-id="41f54-123">Update-ScriptFileInfo cmdlet lets you to update the existing script file metadata.</span></span>
```powershell
...
## <a name="register-psrepository-and-set-psrepository-cmdlets-with-script-sharing-support"></a><span data-ttu-id="41f54-124">Cmdlets Register-PSRepository e Set-PSRepository com suporte ao compartilhamento de script</span><span class="sxs-lookup"><span data-stu-id="41f54-124">Register-PSRepository and Set-PSRepository cmdlets with script sharing support</span></span>
<span data-ttu-id="41f54-125">Use os cmdlets Register-PSRepository/Set-PSRepository para adicionar o **ScriptSourceLocation** e **ScriptPublishLocation** ao PSRepository.</span><span class="sxs-lookup"><span data-stu-id="41f54-125">Use Register-PSRepository/Set-PSRepository cmdlets to add the **ScriptSourceLocation** and **ScriptPublishLocation** to the PSRepository.</span></span>
```powershell
...
## <a name="publish-script-cmdlet"></a><span data-ttu-id="41f54-126">Cmdlet Publish-Script</span><span class="sxs-lookup"><span data-stu-id="41f54-126">Publish-Script cmdlet</span></span>
<span data-ttu-id="41f54-127">O cmdlet Publish-Script permite publicar o arquivo de script com metadados válidos como Versão, Guid, Autor, Descrição, etc.</span><span class="sxs-lookup"><span data-stu-id="41f54-127">Publish-Script cmdlet lets you to publish your script file with valid metadata like Version, Guid, Author, and Description, etc.</span></span>
```powershell
